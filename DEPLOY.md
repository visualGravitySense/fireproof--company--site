...
### Проблема: 404 ошибки при переходе по роутам или URL отображается как `?/path`

**Решение:** 
1. Файл `public/404.html` уже добавлен для поддержки SPA роутинга
2. Проверьте настройку `pathSegmentsToKeep` в `public/404.html`:
   - Если URL вашего сайта: `username.github.io/about` → установите `pathSegmentsToKeep = 0`
   - Если URL вашего сайта: `username.github.io/repo-name/about` → установите `pathSegmentsToKeep = 1`
3. Скрипт в `index.html` автоматически преобразует query string `?/path` обратно в нормальный путь `/path`
